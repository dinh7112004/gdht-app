...
export default function TeacherAIScreen() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      text: "Xin chào Quý Thầy/Cô. Tôi là Cố Vấn Di Sản, rất vinh dự được đồng hành cùng Thầy/Cô trong việc sáng tạo bài giảng và quản lý lớp học. Tôi có thể giúp gì cho Thầy/Cô hôm nay?",
      sender: "ai",
      timestamp: new Date(),
    },
  ]);
  ...
  const teacherPrompts = [
    {
      label: "Soạn giáo án",
      prompt:
        "Gợi ý cho tôi một giáo án dạy về Số học lớp 5 tích hợp với di sản Văn Miếu Quốc Tử Giám.",
    },
    {
      label: "Tạo bài tập",
      prompt:
        "Tạo 5 câu đố toán học vui nhộn liên quan đến trang phục dân tộc Việt Nam.",
    },
    {
      label: "Tư vấn lớp",
      prompt:
        "Làm thế nào để khích lệ học sinh trầm tính tham gia phát biểu trong tiết Toán?",
    },
    {
      label: "Game lớp học",
      prompt:
        "Gợi ý một trò chơi gamification nhanh trong 5 phút để khởi động tiết học.",
    },
  ];
    ...
    try {
      const chat = model.startChat({
        history: [
          {
            role: "user",
            parts: [{ text: GET_TEACHER_SYSTEM_PROMPT(language) }],
          },
          {
            role: "model",
            parts: [
              {
                text: "Tôi đã hiểu nhiệm vụ. Tôi là Cố Vấn Di Sản, sẵn sàng hỗ trợ Quý Thầy/Cô một cách chuyên nghiệp nhất.",
              },
            ],
          },
      ...
      const errorMsg: Message = {
        id: `err-${Date.now()}`,
        text: "Thành thật xin lỗi Thầy/Cô, kết nối với hệ thống cố vấn đang bị gián đoạn. Thầy/Cô vui lòng thử lại sau giây lát.",
        sender: "ai",
        timestamp: new Date(),
      };
  ...
  const clearChat = async () => {
    Alert.alert(
      "Xóa lịch sử",
      "Thầy/Cô muốn xóa toàn bộ nội dung hội thoại này?",
      [
          ...
          onPress: async () => {
            if (userId) {
              ...
              setMessages([
                {
                  id: "1",
                  text: "Lịch sử đã được làm mới. Tôi sẵn sàng hỗ trợ Thầy/Cô.",
                  sender: "ai",
                  timestamp: new Date(),
                },
              ]);
